...
def level_info_2_score_json(
    level_info: group_tools.level_info, matrix_df, matrix_name, gap_frac_cutoff=0.2
):
    ...
    score_json = (
        ...
        / f"conservation_scores_asym_valday_{level_info.level}_{matrix_name}.json"
    )
...
def score_levels_in_og_obj(
    og_obj: group_tools.ortholog_group_info, matrix_name, gap_frac_cutoff, root
):
    ...
    for level in og_obj.levels:
        ...
        score_json = level_info_2_score_json(
            level_info, matrix_df, matrix_name, gap_frac_cutoff=gap_frac_cutoff
        )
        og_obj.add_item_to_level_info(
...
def driver(og_info_json, matrix_name, gap_frac_cutoff, root):
    og_obj = group_tools.ortholog_group_info(og_info_json, root=root)
    og_obj.load_all_level_info_objects()
    score_levels_in_og_obj(og_obj, matrix_name, gap_frac_cutoff, root)
...
def main(
    ...
    if multiprocess:
        ...
        f_args = [(i, matrix_name, gap_frac_cutoff, root) for i in og_info_jsons]
        ...
        for i in og_info_jsons:
            driver(i, matrix_name, gap_frac_cutoff, root)
