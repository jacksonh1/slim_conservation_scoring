"""
changes: change the location of the ortholog_info_json files to a single folder
...
def ortholog_analysis_setup_with_odb_id(
    ...
    odb_main_folder,
    orthogroup_analysis_json_folder,
    root,
    logger,
    regex=None,
):
    ...
    for i in hits_df.index:
        ...
        orthogroup_analysis_json_folder.mkdir(exist_ok=True, parents=True)
        info_dict = {
            "UniprotID": uni_id,
            ...
            "regex": regex,
            "analysis_folder": str(orthogroup_analysis_json_folder.resolve().relative_to(root)),
        }
        try:
        ...
        group_info_json = orthogroup_analysis_json_folder / f"{ref_ind}-{uni_id}_{geneid}_ortholog_group_info.json"
...
def main(
    ...
    orthogroup_analysis_json_folder = Path(orthogroup_analysis_json_folder)
    orthogroup_analysis_json_folder.mkdir(exist_ok=True, parents=True)
    og_analysis_json_folder_rel_to_root = (
        orthogroup_analysis_json_folder.resolve().relative_to(root)
    )
    ...
    (
        ...
        hits_df, odb_og_run_folder, orthogroup_analysis_json_folder, root, logger, regex=regex
    )
    meta_info.META_PARAMS["og_analysis_json_folder"] = str(
        og_analysis_json_folder_rel_to_root
    )
