...
def level_info_2_score_json(
    level_info: group_tools.level_info, output_folder, gap_frac_cutoff
):
    score_json = (
        Path(output_folder)
        / f"{level_info.query_gene_id}_conservation_scores_property_entropy_{level_info.level}.json"
    )
...
def score_levels_in_og_obj(
    og_obj: group_tools.ortholog_group_info, gap_frac_cutoff, output_folder, root
):
    for level in og_obj.levels:
        ...
        score_json = level_info_2_score_json(
            level_info, output_folder=output_folder, gap_frac_cutoff=gap_frac_cutoff
        )
...
def driver(og_info_json, gap_frac_cutoff, output_folder, root):
    # ic(og_info_json)
    og_obj = group_tools.ortholog_group_info(og_info_json, root=root)
    og_obj.load_all_level_info_objects()
    score_levels_in_og_obj(og_obj, gap_frac_cutoff=gap_frac_cutoff, output_folder=output_folder, root=root)
...
def main(
    ...
    if multiprocess:
        ...
        f_args = [(i, gap_frac_cutoff, output_folder, root) for i in og_info_jsons]
        ...
        for i in og_info_jsons:
            driver(i, gap_frac_cutoff, output_folder, root)
            break
