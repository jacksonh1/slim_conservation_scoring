...
import local_orthoDB_analysis_tools_v2.filepaths_env_variables as fp
...
def score_obj_2_motif_match_ave_zscore(hits_df, score_o: group_tools.level_score_class):
    ...
    hit_z_scores = score_obj_2_nongap_scores(score_o, mask=False)
...
def score_obj_2_nongap_scores(score_o: group_tools.level_score_class, mask=True):
    if mask:
        z_score_list = mask_score_list(score_o.z_score_dict['z_scores'], score_o.score_mask)
    else:
        z_score_list = score_o.z_score_dict['z_scores']
    hit_aln_slice = slice(score_o.hit_alignment_start_position, score_o.hit_alignment_end_position+1)
    nongap_positions = cons_tools.get_non_gap_indexes(score_o.query_aligned_sequence_str[hit_aln_slice])
...
def get_MSA_cons_string(score_o):
    ...
    cons_str = cons_tools.conservation_string(hit_z_scores, score_o.hit_sequence_str, z_score_cutoff=1)
    return cons_str
...
def score_obj_2_largest_avg_zscore_for_window(score_o, window_size=5):
    hit_z_scores = score_obj_2_nongap_scores(score_o)
    largest_avg_z = get_largest_avg_zscore_for_window(hit_z_scores, window_size=window_size)
    return largest_avg_z


# def find_highlighted_residues(regex, seq_str):
#     matches = list(tools.get_regex_matches(regex, seq_str))
#     if len(matches) == 0:
#         return None
#     if len(matches) > 1:
#         print(f'Warning: more than one match found for {regex} in {seq_str}, using first match')
#         return list(matches[0][1:])
#     return list(matches[0][1:])

# og_obj = group_tools.ortholog_group_info('../40_ortholog_analysis/21-Q7Z5R6_9606_0:00294e/ortholog_group_info.json')
# og_obj.load_all_level_score_objects_with_z_score(local_sequences=True)
# score_o = og_obj.level_score_objects[og_obj.best_level]

# FLANK=meta_tools.params['flank']

# def score_obj_2_ave_zscore_of_motif(score_o, flank):


# %%

# get_largest_avg_zscore_for_window([0,1,1,2,3,5], window_size=3)
...
def append_dict(score_o: group_tools.level_score_class, annot_dict={}):
    ...
    annot_dict[score_o.reference_index]['full MSA - cons str'] = get_MSA_cons_string(score_o)
    annot_dict[score_o.reference_index]['full MSA - average score of best-scoring 5-residue window'] = score_obj_2_largest_avg_zscore_for_window(score_o, window_size=5)
    annot_dict[score_o.reference_index]['full MSA - average score of best-scoring 10-residue window'] = score_obj_2_largest_avg_zscore_for_window(score_o, window_size=10)
    annot_dict[score_o.reference_index]['full MSA - average score over full window'] = score_obj_2_largest_avg_zscore_for_window(score_o, window_size=len(score_o.hit_sequence_str))
    annot_dict[score_o.reference_index]['full MSA - # points in z-score background'] = len(score_o.z_score_dict['bg_scores'])
    annot_dict[score_o.reference_index]['full MSA - z-score list'] = score_obj_2_nongap_scores(score_o, mask=False)
    annot_dict[score_o.reference_index]['full MSA - motif_match_score'] = score_obj_2_motif_match_ave_zscore(HITS_DF, score_o)
    return annot_dict


def driver(ortholog_group_info_json, level='best', annot_dict={}, local_sequences=False, score_key='asym_valday_score_json'):
...
def main():
    ...
    for og_info_json in og_info_jsons:
        annotation_dict = driver(og_info_json, LEVEL_TO_ANALYZE, annotation_dict, score_key=SCORE_KEY)
