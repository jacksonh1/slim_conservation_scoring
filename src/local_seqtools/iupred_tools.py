...
def main_find_idr_regions(sequence_str: str, iupred_cutoff: float = 0.4, gap_merge_threshold: int = 10, idr_min_length:int = 8) -> list[list[int]]:
    ...
    """    
    idr_regions = get_idr_indexes(iupred_scores_from_seqstring(sequence_str), cutoff=iupred_cutoff)
    idr_regions = merge_idr_regions(idr_regions, gap_merge_threshold=gap_merge_threshold)
