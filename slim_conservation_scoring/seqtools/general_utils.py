...
from slim_conservation_scoring.seqtools import alignment_tools as aln_tools
...
def parse_filename(
    filename,
    regex=r"(?P<species_id>\d+_\d+)_(?P<odbid>[\w\d]+)_(?P<level>\w+)_(?P<og_id>\d+at\d+)",
):
    p = re.compile(regex)
    m = p.match(filename)
...
def str2seqrecord(seq_str, id=None, description=None):
    ...
    if description is None:
        description = ""
    return SeqRecord(Seq.Seq(seq_str), id=id, description=description)
...
# def aligned_2_unaligned_index(seq_str):
#     unal_seq = ""
#     unal_index = -1
#     aln_index_map = []
#     for al_pos, i in enumerate(seq_str):
#         if i == "-":
#             aln_index_map.append(unal_index)
#         else:
#             unal_index += 1
#             aln_index_map.append(unal_index)
#             unal_seq = unal_seq + i
#     # convert -1's to None
#     aln_index_map = [i if i != -1 else None for i in aln_index_map]
#     return unal_seq, aln_index_map
