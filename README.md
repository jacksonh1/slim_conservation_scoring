...
- [Keating lab specific instructions](#keating-lab-specific-instructions)
  - [using the pipeline](#using-the-pipeline-1)
...
I have precalculated a database of orthologs on the Keating lab workstation call Dave. Here are the steps to use the pipeline on Dave (from scratch):
...
I then changed the permissions of the directories `./CODE`, `./orthodb_data`, and `./orthodb_preprocessed_databases` such that anyone in the "slims" group can access them. used commands like: 
`sudo chgrp -R slims ./orthodb_preprocessed_databases/`
`sudo chmod -R g+w orthodb_data/`
