...
for col_name in file_path_columns:
    col_idx = df.columns.get_loc(col_name) + 1  # Get the column index (1-based for openpyxl)
    
    for row_idx, file_path in enumerate(df[col_name], start=2):  # Start at row 2 because row 1 is the header
        if pd.notna(file_path):  # Only add hyperlink if the file path is not NaN
            ...
            cell.font = Font(color="0000FF", underline="single")  # Format as a clickable link
